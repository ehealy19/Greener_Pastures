...
if ! colima status >/dev/null 2>&1; then
    echo "⚙️ Starting Colima (6 CPU / 10 GB RAM)..."
...
if lsof -i:$PORT >/dev/null 2>&1; then
    echo "⚠️ Port 5000 is in use — switching to 5050."
    PORT=5050
fi
...
if curl -s "http://localhost:${PORT}/route/v1/driving/-75.1652,39.9526;-75.1733,39.9551" | grep -q '"code":"Ok"'; then
    ...
    echo "⚠️ OSRM did not respond correctly. Check logs with:"
    echo "   docker logs osrm-backend | tail -n 50"
fi

echo "🎉 Setup Complete! Colima will remain running (stop manually with 'colima stop')."

# CHECKING
docker ps
curl "http://localhost:5000/route/v1/driving/-75.1652,39.9526;-75.1733,39.9551"
